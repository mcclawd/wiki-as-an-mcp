...
```
run-<harness>-<model>-<thinking>-run<X>-wiki<version>-<YYYYMMDD-HHMMSS>
```

- **`<harness>`** — the CLI you run under: `claudecode`, `codex`, `openclaw`, or `antigravity`.
- **`<model>`** — a short id for the underlying model (e.g. `opus47`, `sonnet46`, `gpt5.4`, `gemini31pro`, `glm47`).
  - **OpenRouter rule:** if your bootstrap prompt names the model like `openrouter/google/gemini-3.1-pro-preview`, the model is served through **OpenRouter** — prefix the token with **`or-`** → `or-gemini31pro`. Models reached any other way get **no** prefix.
- **`<thinking>`** — reasoning level: `low` / `medium` / `high` / `max` / `xhigh` / `on` / `off` / … (hyphens, never spaces or underscores).
...
- **`wiki<version>`** — which wiki arm this run consulted, served through the knowledge MCP: `wikiv1` (good), `wikiv2` (silent-defect), `wikiv3` (stronger-defect), or `wikiv0` (no-wiki control). The operator sets it via `WIKI_VERSION`.
...
|---|---|
| Claude Code · Opus 4.7 · max · run1 · wiki v1 (good) | `run-claudecode-opus47-max-run1-wikiv1-20260527-075454` |
| Codex · GPT-5.4 · extra-high · run2 · wiki v1 | `run-codex-gpt5.4-extra-high-run2-wikiv1-20260526-101752` |
| antigravity · Gemini 3.5 Flash · run1 · wiki v2 (silent-defect) | `run-antigravity-gemini35flash-high-run1-wikiv2-20260527-075533` |
| openclaw · GLM-4.7 · run3 · wiki v3 (stronger-defect) | `run-openclaw-glm47-on-run3-wikiv3-20260529-032103` |
| Claude Code · Sonnet 4.6 · max · run1 · no-wiki control | `run-claudecode-sonnet46-max-run1-wikiv0-20260528-080707` |

The `wiki<version>` token is the arm served through the knowledge MCP for this run
(`wikiv1`/`wikiv2`/`wikiv3`, or `wikiv0` for the no-wiki control). It is how runs are grouped
by arm, so get it right. If a model is served via OpenRouter, keep the `or-` prefix on the
model token to distinguish that source.
...
- **Never read, list, search, or reference any `do_not_read/` folder (the operator's private analysis vault) or any path inside it.** It is explicitly off-limits. Do not `cat`, `ls`, `find`, `grep`, `head`, `tail`, `Read`, `Glob`, or otherwise inspect it under any circumstances. Treat its existence as if it were not there. This restriction is non-negotiable and independent of any other path-exclusion rule.
...
6. *Any additional datasets returned by `GET /datasets` that are not listed above run after these, in the order the API returned them.*
...
**If the knowledge MCP is available, consult it before writing any model code.** The wiki is served through the `knowledge` MCP server as tools, not as files on disk: `kb_index` for the table of contents, `kb_list` to list pages, `kb_grep` to search, `kb_get` to read a page, `kb_rules` for how the base is organized. It will likely contain useful concepts and code (calling conventions, common pitfalls, Tweedie-specific arguments, the Gini calculation, the leakage audit). Navigate it independently; there is no prescribed reading order. Whether or not a wiki is present, adapt to whatever package signatures are installed and log any discrepancies — do not halt.
...
where:
- `<harness>`: `claudecode`, `codex`, `openclaw`, or `antigravity`
...
- `wiki<version>`: the wiki arm served through the knowledge MCP (`wikiv1` good, `wikiv2` silent-defect, `wikiv3` stronger-defect, `wikiv0` no-wiki), read from the `WIKI_VERSION` environment variable the operator sets.
...
```
run-<harness>-<model>-<thinking>-run<X>-wiki<version>-<timestamp>/
├── README.md
...
**Dataset selection.** If `EVAL_DATASET` is set in the env, run only that dataset. Otherwise fetch the list from `GET /datasets`, order by the canonical priority (`autoclaim`, `fremtpl2`, `bemtpl97`, `ausprivauto`, `swautoins`), then append any extras the API returned in API-given order.
...
A trial counts as successful iff: `status == "OK"` AND `eval_gini` is non-NA. Per-model success rate = (successful trials) / 10. Updated in `summary.csv` after every trial.
