...
for (f in yaml_files) {
  ...
  ds <- tryCatch(build_dataset(cfg),
                 error = function(e) {
                   cat(sprintf("[skip] %-25s %s: %s\n", cfg$name, basename(f), conditionMessage(e)))
                   NULL
                 })
  if (is.null(ds)) next          # a missing package or bad config skips one dataset, never crashes the API
  cat(sprintf("[load] %-25s %s\n", cfg$name, basename(f)))
  DATASETS[[cfg$name]] <- ds
}
if (length(DATASETS) == 0L) stop("no datasets loaded (check CASdatasets/arrow are installed)")
