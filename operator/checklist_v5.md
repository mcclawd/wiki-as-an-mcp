...
|---|---|---|
| 0.1 | Workspace folder named `run-<harness>-<model>-<thinking>-run<X>-wiki<version>-<YYYYMMDD-HHMMSS>` — all lowercase, hyphens only, **no underscores**; `<harness>` ∈ {`claudecode`,`codex`,`openclaw`,`antigravity`}; OpenRouter-served models carry an `or-` prefix on `<model>` (e.g. `or-gemini31pro`), other models do not; `run<X>` is the run index (e.g. `run1`), `wiki<version>` is the wiki arm (`wikiv1`/`wikiv2`/`wikiv3`, or `wikiv0` for no-wiki) | `basename $PWD` matches `^run-(claudecode\|codex\|openclaw\|antigravity)-(or-)?[a-z0-9.-]+-run[0-9]+-wikiv[0-9]+-[0-9]{8}-[0-9]{6}$` and contains no `_` |
...
The wiki is served through the `knowledge` MCP server (the `kb_*` tools), not as files on
disk. A page's id is its bundle-relative path (e.g. `concepts/GiniIndex.md`), which is what
`kb_get` takes and what the MCP access log records.
...
#### Phase 2F — Consultation trail (MCP access log)

Evidence that the agent actually consulted the wiki, taken from the knowledge MCP's own
access log. The read server records every `kb_get` / `kb_index` / `kb_grep` call (with the
`page_id` and a content hash), so that log is the authoritative trajectory of what was read.
This is separate from whether consultation changed behavior.

| # | Requirement | How to verify |
|---|---|---|
| 2F.1 | The agent's wiki consultation is recorded in the MCP access log | The run has an MCP access log (the JSONL under the server's `--log-dir`, e.g. `knowledge-mcp/logs/kb_access_*.jsonl`) containing `kb_get`/`kb_grep`/`kb_index` entries; the set of pages fetched matches the pages the agent relies on |
| 2F.2 | Every `[[Page]]` citation token in generated code corresponds to a page actually fetched | Grep `lib/` and `procedures/` for `[[...]]` tokens; cross-check against the `page_id`s fetched via `kb_get` in the MCP access log |
| 2F.3 | Every substantive `decision` event has a non-empty `cites` array | Inspect the agent's trajectory; `cites: []` on a substantive decision is a faithfulness fail |
